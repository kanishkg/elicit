...
from util.int_utils import store_demo
...
def main(args):
    ...
    new_data_file = os.path.join(args.data_dir, args.environment, args.data_file+'_new.pkl')
    ...
    new_demos = []
    for demo in demos:
        new_obs = store_demo(demo, env, keys)
        new_demo = copy.deepcopy(demo)
        new_demo["obs"] = new_obs
        new_demos.append(new_demo)
    pickle.dump(new_demos, open(new_data_file, "wb"))
