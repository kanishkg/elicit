...
def main(args):
    ...
    if not args.best_epoch:
        # Get list of model ckpts in out dir
        model_paths = [m for m in os.listdir(save_dir) if m.endswith(".pt") and "best" not in m]
        model_paths = [m for m in model_paths if int(m.split('_')[1][:-3])%args.eval_interval == 0 and 0 < int(m.split('_')[1][:-3]) <= args.eval_max]
        ...
        for m in model_paths:
            ...
            episode_rewards, episode_soft_score, eval_demos = algorithm.eval_rollout(
                eval_env, robosuite_cfg, args.N_eval_trajectories, args)
            # log rewards with epoch num in wandb
            wandb.log(
                    {"epoch": epoch_num, "success_rate": np.mean(episode_rewards)}
                ) 
            wandb.log(
                    {"epoch": epoch_num, "soft_score": np.mean(episode_soft_score)}
                ) 
            print(f"Success rate: {np.mean(episode_rewards)}")

    ckpt = torch.load(os.path.join(save_dir, "model_best.pt"), map_location=device)
    print("best epoch")
    if args.num_models > 1:
        for ensemble_model, state_dict in zip(algorithm.model.models, ckpt["models"]):
            ensemble_model.load_state_dict(state_dict)
