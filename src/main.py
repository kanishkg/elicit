...
def parse_args():
    ...
    parser.add_argument("--save_iter", type=int, default=50, help="Checkpoint will be saved every args.save_iter epochs.")
    ...
    parser.add_argument("--compatibility_type", type=str, default="box", help="Type of compatibility check.")
    ...
    parser.add_argument("--samples_per_epoch", type=int, default=15000, help="Number of samples to use per epoch.")
...
def get_policy(args, env, robosuite_cfg=None):
    if args.environment == "Reach2D":
        ...
        if args.offline_policy:
            ...
            policy = get_policy_data(args.policy)
        else:
...
def main(args):
    ...
    wandb_name = f"{args.environment}-{args.method}-{args.arch}-{args.num_models}-{args.hidden_size}-{args.policy}-{args.data_path.split('/')[-1].split('.')[0]}-{args.N}-{args.seed}-{args.trajectories_per_rollout}-{args.likelihood_filter}-{args.entropy_filter}"
    ...
    if args.eval_only:
        ...
        train, val = get_dataset(args.data_path, args.N, save_dir, args.seq_len)
        ...
        # algorithm.eval_multi_proc(args, robosuite_cfg=robosuite_cfg)
